...
本项目后端使用 **Prisma + SQLite**。数据库就是 `server/prisma/dev.db` 这一个文件，**它不会被提交到 git**（已在 `.gitignore` 中忽略），所以每台设备上的数据库是各自独立的，数据不互通。
...
# 然后编辑 server/.env，填入你的 AI_API_KEY 等配置（DATABASE_URL 保持默认即可）

# 3. 初始化数据库：按顺序执行所有迁移脚本，生成 server/prisma/dev.db
...
第 3 步执行时 Prisma 发现 `dev.db` 不存在，会自动创建空库并从头跑完全部迁移，结束后数据库结构就和最新代码完全一致了。
...
如果别的设备提交了新的数据库改动（新表、新字段），pull 下来后本机的 `dev.db` 就落后了。此时：
...
```
The column `main.User.xxx` does not exist in the current database
```
...
原因：本机 `dev.db` 的实际结构和迁移历史对不上了（通常是违反了上面的铁律，或库文件是旧实验遗留的）。
...
它会删掉 `dev.db`、重建空库、从头跑完所有迁移。代价是本机数据清空（重新注册一个账号即可）。因为 `dev.db` 不进 git，这个操作**不会影响其他设备**。
...
| 场景                  | 命令（项目根目录）                                                                   |
| --------------------- | ------------------------------------------------------------------------------------ |
| 新设备首次搭建        | `npm install` → 建 `server/.env` → `npm run prisma:migrate`                          |
